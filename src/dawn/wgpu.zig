...
pub const compat = struct {
    pub inline fn bufferLabel(text: []const u8) wgpu_common.WGPUStringView {
        return wgpu_common.WGPUStringView{
            .data = text.ptr,
            .length = text.len,
        };
    }

    pub inline fn shaderEntryPoint(text: []const u8) StringView {
        return StringView.fromSlice(text);
    }

    pub inline fn samplerLayout(binding_type: SamplerBindingType) SamplerBindingLayout {
        return SamplerBindingLayout{ .type = binding_type };
    }

    pub inline fn depthWriteEnabled() OptionalBool {
